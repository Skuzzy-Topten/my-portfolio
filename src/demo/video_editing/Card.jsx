  ...
  return (
    <>
      {Data.map((language) => (
        ...
        >
          <div className="video_editing card">
            <img
              src={language.logo && language.logo.image}
              alt={language.logo && language.logo.alt}
              className="video_editing logo"
            />
            <h3 className="video_editing name">{language.name}</h3>
            <h5 className="video_editing type">{language.type}</h5>
            <Accordion type="single" collapsible>
              {language.course && language.course.length > 0 && (
                <AccordionItem value="item-1">
                  <AccordionTrigger className="video_editing course_lists title">
                    Courses
                  </AccordionTrigger>
                  <AccordionContent>
                    <ol className="video_editing course_lists">
                      {language.course &&
